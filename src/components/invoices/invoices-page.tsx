...
export function InvoicesPage({ user, initialView, onInitialViewConsumed }: InvoicesPageProps) {
  ...
  const [showEInvoiceSendStatus, setShowEInvoiceSendStatus] = useState<string | null>(null);
  const [einvoiceConfig, setEinvoiceConfig] = useState<any>(null);
  ...
  const fetchEInvoiceConfig = useCallback(async () => {
    try {
      const response = await fetch('/api/company/einvoice-settings');
      if (response.ok) {
        const data = await response.json();
        setEinvoiceConfig(data.settings || null);
      }
    } catch { /* silent */ }
  }, []);
  ...
  // as a swipe neighbor) and the user navigates here via a quick action that
  // passes initialView='create'. useState only uses the initial value on first
  ...
  // Consume the initialView prop once it's been applied
  useEffect(() => {
    if (initialView && onInitialViewConsumed) {
      onInitialViewConsumed();
    }
  }, [initialView, onInitialViewConsumed]);
    ...
    try {
      ...
      if (!response.ok) {
        const isAccess = await handleMutationError(
          response,
          language === 'da' ? 'Gem virksomhedsoplysninger' : 'Save company info'
        );
  ...
  }, [companyForm, t, language, handleMutationError]);
  ...
  const handleDeleteInvoice = useCallback(async (id: string) => {
    try {
      const deleteRes = await fetch(`/api/invoices?id=${id}`, { method: 'DELETE' });
      if (!deleteRes.ok) {
        const isAccess = await handleMutationError(
          deleteRes,
          language === 'da' ? 'Slet faktura' : 'Delete invoice'
        );
      } else {
  ...
  }, [t, language, fetchInvoices, handleMutationError]);
  ...
  const renderInvoicePreview = () => {
    ...
    const lineItems = previewInvoice.lineItems as LineItem[];
                ...
                {previewInvoice.status !== 'PAID' && previewInvoice.status !== 'CANCELLED' && (
                  <>
                    {previewInvoice.status === 'DRAFT' && (
                      <>
                        <Button
                          ...
                          className="gap-1.5 text-gray-600 dark:text-gray-300 h-8 px-2 sm:px-3"
                        >
                          <Pencil className="h-3.5 w-3.5" />
                          <span className="hidden md:inline">{language === 'da' ? 'Rediger kladde' : 'Edit Draft'}</span>
                        </Button>
                        <Button
                          variant="outline"
                          size="sm"
                          onClick={() => { setSendDialogInvoice(previewInvoice); }}
                          className="gap-1.5 text-[#7dabb5] dark:text-[#80c0cc] h-8 px-2 sm:px-3"
                        >
                          <Send className="h-3.5 w-3.5" />
                          <span className="hidden md:inline">{language === 'da' ? 'Send' : 'Send'}</span>
                        </Button>
                      </>
                    )}
                    {previewInvoice.status === 'SENT' && (
                      ...
                      >
                        <Send className="h-3.5 w-3.5" />
                        <span className="hidden md:inline">{language === 'da' ? 'Send igen' : 'Send Again'}</span>
                      </Button>
                    )}
                    <Button
                      variant="outline"
                    ...
                    >
                      ...
                      <span className="hidden md:inline">{t('markAsPaid')}</span>
                    </Button>
                  </>
                )}
                  ...
                  <DropdownMenu>
                    ...
                    <DropdownMenuContent align="end" className="w-52">
                      <DropdownMenuLabel className="text-xs text-muted-foreground">
                        {language === 'da' ? 'Dokument' : 'Document'}
                      </DropdownMenuLabel>
                      <DropdownMenuItem onClick={() => handlePrintInvoice(previewInvoice)}>
        ...
        <Card className="stat-card border-0 shadow-lg dark:border dark:border-white/5">
          ...
          <CardContent className="flex flex-col">
            <div className="rounded-lg border border-gray-200 dark:border-white/10 bg-white dark:bg-[#141918] overflow-hidden">
              ...
              >
                <div className="invoice-preview-content" style={{ fontFamily: "-apple-system, BlinkMacSystemFont, 'Segoe UI', sans-serif", padding: '72px 80px' }}>
        ...
        action={
          <div className="flex items-center gap-2">
            ...
            <Button
                ...
                className="bg-transparent text-[#0d9488] border border-[#0d9488]/40 hover:bg-[#0d9488] hover:text-white dark:text-[#2dd4bf] dark:border-[#2dd4bf]/40 dark:hover:bg-[#2dd4bf] dark:hover:text-[#0d9488] lg:text-white lg:border-white/40 lg:hover:bg-white/20 gap-2"
              >
    ...
    <>
      ...
      {sendEInvoiceInvoice && (
        <SendEInvoiceDialog
          ...
          onClose={() => setSendEInvoiceInvoice(null)}
          onSuccess={() => {
            setSendEInvoiceInvoice(null);
            fetchInvoices();
          }}
        />
      )}
      {showEInvoiceSendStatus && (
        <Dialog open={!!showEInvoiceSendStatus} onOpenChange={() => setShowEInvoiceSendStatus(null)}>
          <DialogContent className="flex flex-col max-w-5xl w-[95vw] sm:w-[90vw] max-h-[85vh] overflow-y-auto">
            <DialogHeader>
              <DialogTitle>{language === 'da' ? 'E-faktura send-historik' : 'E-invoice send history'}</DialogTitle>
            </DialogHeader>
            <EInvoiceSendStatus invoiceId={showEInvoiceSendStatus} />
          </DialogContent>
        </Dialog>
      )}
    </>
