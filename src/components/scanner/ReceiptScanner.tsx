      ...
      <div className={`relative flex-1 ${showResult ? 'hidden' : ''}`}>
        ...
        {phase === 'processing' && (
          <div className="absolute inset-0 z-20 flex flex-col items-center justify-center bg-black p-6 text-center">
            ...
            <p className="text-white/80 text-sm font-medium">{t('processingReceipt')}</p>
            <p className="text-xs text-white/40 mt-1.5">{t('adjustingPerspective')}</p>
          </div>
        )}
          ...
          <div className="shrink-0 relative z-10 border-t border-gray-200 dark:border-white/10 bg-white dark:bg-[#1a1f1e] px-3 sm:px-4 pt-3 pb-[max(1.5rem,env(safe-area-inset-bottom))]">
            <div className="flex items-center gap-2 sm:gap-3 max-w-full mx-auto min-w-0">
              ...
              >
                ...
                <span className="hidden sm:inline">{t('tryAgain')}</span>
              </button>
            </div>
          </div>
        </div>
...
function LoadingScreen() {
  ...
  return (
    <div className="flex flex-col items-center justify-center h-full p-6 text-center">
      ...
      <p className="text-sm text-white/80 font-medium">{t('loadingScanner')}</p>
      <p className="text-xs text-white/40 mt-1.5">{t('preparingRecognition')}</p>
    </div>
  );
}
...
function PermissionScreen() {
  ...
  return (
    <div className="flex flex-col items-center justify-center h-full p-6 text-center">
      ...
      <p className="text-sm text-white/80 font-medium">{t('waitingCameraPermission')}</p>
      <p className="text-xs text-white/40 mt-1.5">{t('allowCameraBrowser')}</p>
    </div>
  );
}
