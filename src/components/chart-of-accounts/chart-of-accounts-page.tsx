...
export function ChartOfAccountsPage({ user, onNavigate }: ChartOfAccountsPageProps) {
  ...
  const handleSeed = useCallback(async () => {
    ...
    try {
      ...
      if (!response.ok) {
        const isAccess = await handleMutationError(response, isDanish ? 'Opret standardkonti' : 'Seed accounts');
        if (isAccess) { setIsSeeding(false); return; }
    ...
    try {
      const response = await fetch('/api/accounts', {
        ...
        body: JSON.stringify({
          ...
          postingGuide: formData.postingGuide.trim() || undefined,
        }),
      });

      if (!response.ok) {
        const data = await response.json().catch(() => ({}));
        const isAccess = await handleMutationError(response, isDanish ? 'Opret konto' : 'Create account');
      ...
      if (!(err instanceof Error && err.message === '__access_denied__')) {
    ...
    try {
      const response = await fetch(`/api/accounts/${editAccount.id}`, {
        ...
        body: JSON.stringify({
          ...
          postingGuide: formData.postingGuide.trim() || null,
        }),
      });

      if (!response.ok) {
        const data = await response.json().catch(() => ({}));
        const isAccess = await handleMutationError(response, isDanish ? 'Opdater konto' : 'Update account');
        if (isAccess) { setIsSaving(false); return; }
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      if (!response.ok) {
        const data = await response.json().catch(() => ({}));
        const isAccess = await handleMutationError(response, isDanish ? 'Slet konto' : 'Delete account');
        if (isAccess) { setDeleteTarget(null); return; }
  ...
  return (
    <div className="p-3 lg:p-6 space-y-4 lg:space-y-6">
                ...
                <button
                  ...
                  className={`flex items-center gap-1.5 px-3 py-2 rounded-lg text-xs sm:text-sm font-medium transition-colors whitespace-nowrap ${
                    isActive
                      ...
                      : 'text-gray-500 hover:text-gray-700 dark:text-gray-400 dark:hover:text-gray-300 hover:bg-gray-50 dark:hover:bg-white/[0.04]'
                  }`}
                >
            ...
            {activeTab === 'accounts' && (
              <div className="flex items-center gap-2 shrink-0">
                ...
                <Button
                  ...
                  variant="outline"
                  size="sm"
                  className="gap-1.5 bg-gray-200 hover:bg-gray-300 text-gray-700 border border-gray-300 lg:bg-white/10 lg:hover:bg-white/20 lg:text-white lg:border-white/20"
                >
                  {isSeeding ? <Loader2 className="h-3.5 w-3.5 animate-spin" /> : <RotateCcw className="h-3.5 w-3.5" />}
                ...
                >
                  ...
                  <span className="hidden sm:inline">{isDanish ? 'Ny konto' : 'New'}</span>
                </Button>
              </div>
            )}
          </div>
      ...
      {/* Tab Content */}
      {activeTab === 'mapping' && <StandardMappingPanel user={user} />}
      {activeTab === 'vat' && <VATMappingPanel />}
      {activeTab === 'guide' && <PostingGuideAssistant user={user} />}
          ...
          <div className="space-y-2">
            ...
            <Textarea
              ...
              placeholder={isDanish
                ...
                : 'Add posting guide for this account. E.g.: "Used for domestic service sales with 25% VAT"'}
              value={formData.postingGuide}
              onChange={(e) => onChangeFormData((f) => ({ ...f, postingGuide: e.target.value }))}
              rows={3}
              className="bg-gray-50 dark:bg-white/5"
            />
            <p className="text-[10px] text-gray-400 dark:text-gray-500">
              {isDanish
                ? 'Denne vejledning vises ved bogføring for at hjælpe brugeren med at vælge den rigtige konto.'
                : 'This guide is shown during posting to help the user choose the right account.'}
            </p>
          </div>
