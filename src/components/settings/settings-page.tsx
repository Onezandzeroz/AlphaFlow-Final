...
export function SettingsPage({ user, onNavigate }: SettingsPageProps) {
  ...
  // Read initial tab from URL search params (e.g. /settings?tab=access)
  ...
  // ── React to popstate changes while already mounted (e.g. modal navigates to /settings?tab=access) ──
  useEffect(() => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        await handleMutationError(
          response,
          language === 'da' ? 'Gem præferencer' : 'Save preferences'
        );
      }
    } catch {
  ...
  }, [prefs, t, handleMutationError, language]);
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab} className="space-y-4 lg:space-y-6">
        <TabsList className="bg-white/80 dark:bg-[#1a1f1e]/80 backdrop-blur-sm border border-gray-200 dark:border-white/10 rounded-xl p-1">
          ...
          >
            ...
            <span className="hidden sm:inline">{language === 'da' ? 'E-faktura' : 'E-invoice'}</span>
          </TabsTrigger>
          <TabsTrigger
            value="defaults"
            className="gap-2 rounded-lg data-[state=active]:bg-teal-50 data-[state=active]:text-[#0d9488] data-[state=active]:shadow-sm data-[state=active]:shadow-[#0d9488]/20 data-[state=active]:border data-[state=active]:border-[#0d9488]/20 transition-all relative"
        ...
        {/* ═══ E-INVOICE SETTINGS TAB (FASE 4) ═══ */}
        <TabsContent value="einvoice" className="border-l-4 border-[#0d9488] dark:border-[#2dd4bf] pl-4">
              ...
              <CardContent className="space-y-6">
                ...
                <div className="space-y-3">
                  ...
                  >
                    ...
                    <SelectContent>
                      <SelectItem value="Netto 8 dage">{t('net8Days')}</SelectItem>
                      <SelectItem value="Netto 14 dage">{t('net14Days')}</SelectItem>
                      <SelectItem value="Netto 30 dage">{t('net30Days')}</SelectItem>
                      <SelectItem value="Netto 60 dage">{t('net60Days')}</SelectItem>
                    </SelectContent>
                  </Select>
