 ...
 *  When journalVAT is available (authoritative from double-entry), use it.
 *  Otherwise fall back to the transaction's own vatPercent field:
 *    - Virtual transactions (id starts with "inv-") carry NET amounts → VAT = net × rate / 100
        ...
        action={
          <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
            <DialogTrigger asChild>
              <Button className="bg-[#0d9488] hover:bg-[#0f766e] text-white border border-[#0d9488] lg:bg-white/20 lg:hover:bg-white/30 lg:border-white/30 lg:backdrop-blur-sm gap-2">
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          <div className="flex items-center gap-1 mt-1 flex-wrap">
                            ...
                            {/* Unified kind badge — green (money in) / red (money out).
                                Pile-ikonet for PURCHASE er grønt (received), resten styres af typeInfo.badgeClass. */}
                            <Badge className={cn("text-[10px] px-1.5 py-0 border-0 gap-1", typeInfo.badgeClass)}>
                              <TypeIcon className={cn(
                                "h-2.5 w-2.5",
                                // KUN pile-ikonet for PURCHASE gøres grønt (received = positivt signal).
                                // Badge-baggrund + amount text forbliver styret af typeInfo.badgeClass / amountClass.
                                getTransactionKind(transaction) === 'PURCHASE' && "text-green-600 dark:text-green-400"
                              )} />
                              {typeInfo.label}
                            </Badge>
                            ...
                            {transaction.bankReconciled === false && !transaction.id.startsWith('inv-') && (
                              <Badge className="text-[10px] px-1.5 py-0 border-0 gap-1 bg-amber-500/10 text-amber-600 dark:bg-amber-500/20 dark:text-amber-400">
                          ...
                          <div className="flex items-center gap-0.5">
                            {transaction.receiptImage ? (
                              ...
                              <Button
                                ...
                                className="h-8 w-8 p-0 text-gray-500 hover:text-[#0d9488] dark:text-gray-400 dark:hover:text-[#2dd4bf] hover:bg-[#0d9488]/10"
                              >
                            ...
                            <AlertDialog>
                              <AlertDialogTrigger asChild>
                                <Button
                                  variant="ghost"
                                  size="sm"
                                  className="h-8 w-8 p-0 text-gray-500 hover:text-red-500 dark:text-gray-400 hover:bg-red-50 dark:hover:bg-red-500/10"
                                >
                    ...
                    >
                      ...
                      <TableCell className="max-w-[150px] lg:max-w-[250px] truncate">
                        <div className="flex items-center gap-1.5">
                          ...
                          {transaction.bankReconciled === false && !transaction.id.startsWith('inv-') && (
                            <Badge className="shrink-0 text-[10px] px-1.5 py-0 border-0 gap-1 bg-amber-500/10 text-amber-600 dark:bg-amber-500/20 dark:text-amber-400">
                              {language === 'da' ? 'Ubetalt' : 'Unpaid'}
                            </Badge>
                          )}
                          {transaction.project && (
                                ...
                                <Tooltip>
                                  <TooltipTrigger asChild>
                                    <Button
                                      ...
                                      size="sm"
                                      onClick={() => openUploadDialog(transaction.id)}
                                      className="text-gray-400 hover:text-[#0d9488] dark:text-gray-500 dark:hover:text-[#2dd4bf]"
                                    >
                                      <Paperclip className="h-4 w-4" />
                                    </Button>
                                  </TooltipTrigger>
                              ...
                              <Tooltip>
                                <TooltipTrigger asChild>
                                  <Button
                                    ...
                                    onClick={() => handleExportPeppol(transaction.id)}
                                    disabled={exportingId === transaction.id}
                                    className="text-gray-500 hover:text-[#0d9488] dark:text-gray-400"
                                  >
                                    <FileText className="h-4 w-4" />
                                  </Button>
                                </TooltipTrigger>
