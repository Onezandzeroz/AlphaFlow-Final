...
import { Loader2, ArrowRight, Shield, Lock, Zap, Mail, ArrowLeft, Check, Smartphone, KeyRound } from 'lucide-react';
...
// ─── Two-Factor Verification Prompt (during login) ───
function TwoFactorPrompt({ email, onSuccess, onBack }: { email: string; onSuccess: (user: User) => void; onBack: () => void }) {
  ...
  const handleSubmit = useCallback(async (e: React.FormEvent) => {
    ...
    try {
      ...
      setError(data.error || (isDanish ? 'Ugyldig kode' : 'Invalid code'));
    } catch {
      setError(isDanish ? 'Der opstod en fejl' : 'An error occurred');
    } finally {
      setIsLoading(false);
    }
  }, [email, code, backupCode, useBackup, onSuccess, isDanish]);

  return (
    <div className="space-y-5 py-2">
      {/* Shield icon */}
      <div className="flex justify-center">
        <div className="h-16 w-16 rounded-full bg-teal-50 dark:bg-teal-900/20 flex items-center justify-center">
      ...
      <div className="text-center space-y-2">
        <h3 className="text-lg font-semibold text-slate-900 dark:text-white">
          {isDanish ? 'Tofaktor-bekræftelse' : 'Two-Factor Authentication'}
        </h3>
        <p className="text-sm text-slate-600 dark:text-slate-400 leading-relaxed">
          {isDanish
            ? 'Indtast koden fra din autentificerings-app for at fortsætte.'
            : 'Enter the code from your authenticator app to continue.'}
        </p>
      </div>

      {/* Email card */}
      <div className="bg-teal-50 dark:bg-teal-900/20 border border-teal-200 dark:border-teal-800/40 rounded-xl px-4 py-3 text-center">
        <p className="text-sm font-semibold text-teal-800 dark:text-teal-300 break-all">{email}</p>
      </div>
      ...
      {!useBackup ? (
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          >
            {isLoading ? (
              ...
              <>
                ...
                {isDanish ? 'Bekræft kode' : 'Verify Code'}
              </>
            )}
          </Button>
          ...
          <button
            ...
            className="w-full text-center text-xs text-gray-500 hover:text-[#0d9488] dark:text-gray-400 dark:hover:text-teal-400 transition-colors"
          >
            {isDanish ? 'Brug backup-kode i stedet' : 'Use backup code instead'}
          </button>
        </form>
      ) : (
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="space-y-2">
            <Label className="text-gray-700 dark:text-gray-300 text-sm font-medium flex items-center gap-2">
              ...
              {isDanish ? 'Backup-kode' : 'Backup Code'}
            </Label>
            <Input
              type="text"
              ...
              maxLength={8}
              disabled={isLoading}
              className="h-11 text-center font-mono text-lg uppercase tracking-wider focus-ring-teal bg-white/60"
              autoComplete="off"
            />
            <p className="text-xs text-gray-500 dark:text-gray-400">
              {isDanish
                ? 'Indtast en af dine 8-cifrede backup-koder.'
          ...
          >
            {isLoading ? (
              ...
              <>
                ...
                {isDanish ? 'Brug backup-kode' : 'Use Backup Code'}
              </>
            )}
          </Button>
          ...
          <button
            ...
            className="w-full text-center text-xs text-gray-500 hover:text-[#0d9488] dark:text-gray-400 dark:hover:text-teal-400 transition-colors"
          >
            {isDanish ? 'Tilbage til app-kode' : 'Back to app code'}
          </button>
        </form>
      )}
...
export function LoginForm({ onSuccess, onSwitchToRegister }: LoginFormProps) {
  ...
  const [twoFactorState, setTwoFactorState] = useState<{ email: string } | null>(null);
  const { t, isDanish } = useTranslation();
  ...
  const handleSubmit = useCallback(
    async (e: React.FormEvent) => {
      ...
      try {
        ...
        // Check if 2FA is required (this is a 200 response with requiresTwoFactor flag)
        if (data.requiresTwoFactor) {
          setTwoFactorState({ email: data.email });
          return;
        }
  ...
  if (twoFactorState) {
    return <TwoFactorPrompt email={twoFactorState.email} onSuccess={onSuccess} onBack={() => { setTwoFactorState(null); setError(''); }} />;
  }
