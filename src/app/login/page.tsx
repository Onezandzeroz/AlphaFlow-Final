...
const VIEW_TITLES_EN: Record<View, string> = {
  ...
  'invoices': 'Sales & Invoice — E-invoicing & Peppol',
...
// The SPA now lives at /login — the dashboard view maps to /login, all
// other views map to /<view> (served via the catch-all rewrite → /login).
function viewToPath(view: View, search?: string): string {
  const base = view === 'dashboard' ? '/login' : `/${view}`;
          ...
          <p className="mt-3 flex items-center justify-center flex-wrap gap-x-3 gap-y-1">
            {[
              ...
              <a
                key={link.href}
                href={link.href}
                className="text-[11px] text-gray-400 hover:text-[#0d9488] transition-colors"
              >
                {link.label}
              </a>
            ))}
