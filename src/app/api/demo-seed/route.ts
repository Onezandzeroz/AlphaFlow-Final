  ...
  for (const inv of dynamicInvoices) {
    ...
    await db.invoice.create({
      data: {
        ...
        lineItems: lineItemsWithAccountId,
        subtotal,
        vatTotal,
        total,
        currency: 'DKK',
  ...
  try {
    ...
    if (existingDemoTransactions > 0) {
      ...
      const [transactions, invoices, journalEntries, contacts, companyInfo, fiscalPeriods] = await Promise.all([
        db.transaction.count({ where: { ...tenantFilter(ctx) } }),
        db.invoice.count({ where: { ...tenantFilter(ctx) } }),
        db.journalEntry.count({ where: { ...tenantFilter(ctx) } }),
        db.contact.count({ where: { ...tenantFilter(ctx) } }),
        db.company.count({ where: { ...tenantFilter(ctx), isDemo: true } }),
        db.fiscalPeriod.count({ where: { ...tenantFilter(ctx) } }),
      ]);
