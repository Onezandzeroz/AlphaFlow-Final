...
export const GET = withGuard(
  ...
  async (request, ctx) => {
    try {
      ...
      const contactsToExport = tenantContacts.length > 0
        ? tenantContacts
        : [];
      ...
      if (contactsToExport.length > 0) {
        contactsToExport.forEach(contact => {
          ...
          if (contact.address || contact.city || contact.postalCode) {
            ...
            if (contact.country) addressNode.ele('Country').txt(contact.country);
          }
        });
      } else {
        const customerNode = customers.ele('Customer');
