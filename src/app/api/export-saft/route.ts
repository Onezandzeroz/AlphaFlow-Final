...
export const GET = withGuard(
  ...
  async (request, ctx) => {
    try {
      ...
      // ═══════════════════════════════════════════════════════════════
      // 2. MasterFiles (M 1..1) — children in fixed XSD order:
      //    GeneralLedgerAccounts (M), TaxTable (M), Customers (MIFU),
      //    Suppliers (MIFU), ...
      // ═══════════════════════════════════════════════════════════════
      ...
      // ── 2b. TaxTable (M 1..1) — renamed from TaxCodeTable in v2.1 ──
      // Each TaxCodeDetails has: TaxCode (system), StandardTaxCode (official),
      // Description, TaxPercentage, Country
      const taxTable = masterFiles.ele('TaxTable');

      const mappedVatCodes = getMappedSaftVatCodes();
      mappedVatCodes.forEach((vc) => {
        const taxCodeDetails = taxTable.ele('TaxCodeDetails');
        // TaxCode = AlphaFlow's internal code (system-specific)
      ...
      // Customers = contacts with type CUSTOMER or BOTH
      ...
      if (customerContacts.length > 0) {
        customerContacts.forEach(contact => {
          ...
          customerNode.ele('CompanyName').txt(contact.name);
          if (contact.cvrNumber) {
            customerNode.ele('CustomerTaxID').txt(
              contact.cvrNumber.startsWith('DK') ? contact.cvrNumber : `DK${contact.cvrNumber}`,
            );
          }
          const addressNode = customerNode.ele('Address');
          if (contact.address) addressNode.ele('StreetName').txt(contact.address);
          if (contact.city) addressNode.ele('City').txt(contact.city);
          if (contact.postalCode) addressNode.ele('PostalCode').txt(contact.postalCode);
        ...
        customerNode.ele('CompanyName').txt('General Customers');
      }

      // ── 2d. Suppliers (MIFU) — NEW in this compliance update ──
      // Previously missing entirely. Now generated from contacts with type SUPPLIER or BOTH.
      ...
      if (supplierContacts.length > 0) {
        ...
        supplierContacts.forEach(contact => {
          ...
          supplierNode.ele('CompanyName').txt(contact.name);
          if (contact.cvrNumber) {
            supplierNode.ele('SupplierTaxID').txt(
              contact.cvrNumber.startsWith('DK') ? contact.cvrNumber : `DK${contact.cvrNumber}`,
            );
          }
