...
export async function GET(request: NextRequest) {
  try {
    ...
    // ─── Data integrity: compute SHA-256 checksum of export data (excluding filesData) ───
    const integrityObject = { exportData, summary };
    const integrityJson = JSON.stringify(integrityObject);
    const dataChecksum = crypto.createHash('sha256').update(integrityJson).digest('hex');

    // Update meta with checksum
    meta.dataChecksum = dataChecksum;
    meta.checksumAlgorithm = 'sha256';
    ...
    return NextResponse.json({ exportData, summary, filesData, dataChecksum });
  } catch (error) {
