...
export const GET = withGuard({
  ...
  try {
    ...
    if (record?.sidebarPrefs) {
      sidebarPrefs = record.sidebarPrefs as { expandedSections?: string[] } | null;
    }

    let userPrefs: UserPreferences = {};
    if (record?.userPrefs) {
      userPrefs = record.userPrefs as UserPreferences;
    }
  ...
  try {
    ...
    if (Array.isArray(expandedSections)) {
      ...
      await db.user.update({
        where: { id: ctx.id },
        data: {
          sidebarPrefs: { expandedSections: filtered },
        },
      });
    }
    ...
    if (Object.keys(userPrefs).length > 0) {
      ...
      if (record?.userPrefs) {
        existingPrefs = record.userPrefs as UserPreferences;
      }
