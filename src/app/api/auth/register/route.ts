...
export const POST = withGuard({ auth: false }, async (request: NextRequest) => {
  try {
    ...
    // NemHandel registration-notice email is NO LONGER sent at registration.
    // It is now gated: it fires for ALL tenants (any plan) AFTER they save
    // their company information with a verified CVR (see
    // maybeNotifyNemHandelAfterCompanySave in src/lib/nemhandel-notification.ts,
    // triggered from PUT /api/company and /api/cvr/lookup). NemHandel/OIOUBL
    // is offered to every customer — no plan-tier gate. New tenants have no
    // verified CVR at registration time, so they never qualify here anyway.
