...
>
> **Version:** 1.3 — 2026 · **Ansvarlig:** AlphaAi Consult ApS
>
> **Opdateret v1.3:** Underbehandler-DPA'er samlet til ét bilagspunkt (Bilag 13). Tjeklisten renummereret til Bilag 14.
>
...
> **Vigtigt vedr. underbehandler-DPA'er (Bilag 13):** De faktiske, underskrevne databehandleraftaler med AlphaFlows underbehandlere vedhæftes som **separate PDF-filer ved indsendelsen via virk.dk**. Aftalerne opbevares ikke i kildekoderepositoriet, da de indeholder fortrolige kontraktvilkår. Nedenfor i afsnit 3 resumeres hver aftales parter, formål og behandlingsomfang, så henvisningerne i tjeklisten kan følges. Ved indsendelsen vedlægges de underskrevne PDF'er samlet som Bilag 13.
...
| **2** | Compliance-rapport | `docs/Bilag-02_Compliance-rapport.md` | Krav-for-krav kortlægning af Bogføringsloven + BEK 97/98 + GDPR |
| **3** | Krypteringsrapport | `docs/Bilag-03_Krypteringsrapport.md` | Teknisk krypteringsdokumentation (AES-256-GCM, bcrypt, TLS, nøglehåndtering) |
| **4** | Brugsvejledning | `docs/Bilag-04_Brugsvejledning.md` | Brugermanual for alle funktioner |
| **5** | Databehandleraftale (hoved) | `docs/Bilag-05_Databehandleraftale.md` | DPA mellem AlphaAi Consult ApS (databehandler) og kunden (dataansvarlig) — GDPR Art. 28 |
| **6** | Risikovurdering (DPIA) | `docs/Bilag-06_Risikovurdering-DPIA.md` | IT-risikovurdering jf. BEK 97 §8 stk. 2 + GDPR Art. 35 |
| **7** | Beredskabsplan | `docs/Bilag-07_Beredskabsplan.md` | Disaster Recovery — RTO/RPO, gendannelsesprocedurer, incident response |
| **8** | Leverandørstyring | `docs/Bilag-08_Leverandørstyring.md` | Leverandørevurdering + Transfer Impact Assessments (USA-overførsler) |
| **9** | IT-sikkerhed Neon & IONOS | `docs/Bilag-09_IT-sikkerhed-Neon-og-IONOS.md` | Tredjeparts IT-sikkerhedsdokumentation for infrastruktur |
| **10** | Udbedringsplan | `docs/Bilag-10_Udbedringsplan.md` | Plan for videreudvikling af platformen |
| **11** | TokenPay/TokenBay-guide | `docs/Bilag-11_TokenPay-TokenBay-guide.md` | Adgangsstyring og miljøvariabel-guide |
| **12** | Bilagsoversigt | `docs/Bilag-12_Bilagsoversigt.md` | Dette dokument |
| **13** | Databehandleraftaler med underbehandlere | separat PDF ved indsendelse | Underskrevne DPA'er for AlphaFlows underbehandlere — vedhæftes samlet ved indsendelsen |
| **14** | Tjekliste | `docs/AlphaFlow_Endelig-Tjekliste.xlsx` | Tjekliste til anmeldelsen (BEK 98) |

> Bilag 1–12 og 14 udgør den skriftlige dokumentation i repositoriet. Bilag 13 er de underskrevne underbehandler-DPA'er, der vedhæftes som separate PDF-filer ved indsendelsen.

---

## 3. Underbehandler-DPA'er (Bilag 13) — detaljeret oversigt

Bilag 13 udgøres af de underskrevne databehandleraftaler med AlphaFlows underbehandlere. For hver underbehandler angives nedenfor parter, formål, datakategorier, behandlingslokation, DPA-status og SCC-status. Den tekniske detaljebeskrivelse findes i Bilag 8 (Bilag-08_Leverandørstyring.md), og de USA-baserede overførelsers Transfer Impact Assessments findes i Bilag 8 afsnit 5. De underskrevne DPA'er vedhæftes samlet som Bilag 13 ved indsendelsen.
...
- **Data minimization:** Per-tenant opt-in (`HermesAgent.dataAccessEnabled`, default false) for Hermes chat; dokument-upload opt-in for knowledge-RAG; VLM kaldes kun ved low OCR-confidence — jf. Bilag 5 (Bilag-05_Databehandleraftale.md) afsnit 6.1–6.3.
- **Konfigurationsnote (OPENAI_API_KEY):** `OPENAI_API_KEY` er en valgfri alternativ embedding-udbyder; i den dokumenterede produktionskonfiguration er KUN `OPENROUTER_API_KEY` sat, så al AI-data går via OpenRouter (Bilag 13). Hvis `OPENAI_API_KEY` sættes, vil knowledge-service embeds gå direkte til OpenAI (USA), hvilket kræver en separat OpenAI DPA + SCC.
...
Følgende myndigheder modtager data i forbindelse med lovpligtige indberetninger. De er **ikke** underbehandlere og indgår ikke i Bilag 13:
...
| z-ai-web-dev-sdk (legacy AI-SDK) | Sandbox-only — inaktiv | Legacy sandbox-SDK; fejler graceful i produktion. AI-bankafstemning er nu implementeret i produktion via OpenRouter (`src/lib/matching-engine.ts`) og udveksler persondata via OpenRouter (Bilag 13) — se ikke denne række for AI-bankafstemning. SDK'et selv udveksler ingen data. |
