...
if (clientAlreadyExists) {
  ...
  if (!ok) {
    console.warn('[Hermes Boot] First generate attempt failed, retrying in 2s...')
    execSync('sleep 2', { stdio: 'pipe' })
    const retryOk = runGenerate()
    if (!retryOk) {
      console.error('[Hermes Boot] prisma generate failed twice — cannot start service.')
  ...
  if (!existsSync(clientIndexPath)) {
    console.error('[Hermes Boot] prisma generate reported success but client file not found:')
    console.error(`[Hermes Boot]   Expected: ${clientIndexPath}`)
    console.error('[Hermes Boot] This may be a schema output path issue. Try running manually:')
    console.error('[Hermes Boot]   cd ~/var/www/AlphaFlow-Final && bunx prisma generate')
    process.exit(1)
  }
